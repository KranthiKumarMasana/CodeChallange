...
namespace CodingChallenge.DataAccess
{
    public class LibraryService : ILibraryService
    {
        ...
        public IEnumerable<Movie> SearchMoviesByFranchise(string franchise, int? skip = null, int? take = null, string sortColumn = null, SortDirection sortDirection = SortDirection.Ascending)
        {
            //var movies = GetMovies().Where(s => s.Title.ToLower().Contains(title));
            var movies = GetMovies().Where(x => x.Franchise.ToLower() == franchise.ToLower());
            if (skip.HasValue && take.HasValue)
            {
                movies = movies.Skip(skip.Value).Take(take.Value);
            }
